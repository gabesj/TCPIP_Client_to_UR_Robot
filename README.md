...
2. The operator presses the main Reset button
...
### Safety:
I'd like to emphasize that by using the methods I am describing, you need to be __sure__ that it is in fact safe for the robot to return to operation.  It is your responsibility as the integrator.  Resetting safety circuits/relays requires a manual reset for a reason.  In the partially completed machine pictured below, the machine can ensure that the robot's area is clear through use of guarding, safety light curtains, and safety area scanners.  
...
If you don't have a preferred compiler or aren't sure exactly what to do, I'll outline the steps that I used.  The computer on the machine was running Window 7, so I compiled the .exe programs on a Windows 7 computer - do likewise.  Install a text editor (like Notepad++ or one of many others).  Install MinGW (there are many online tutorials avaialble).  Copy and paste the "TCPIP_SocketClient.cpp" into the .\MinGW\bin directory.  Using the command prompt terminal, `cd` to the bin directory.  Then type `g++ TCPIP_SocketClient.cpp -o TCPIP_SocketClient.exe -l ws2_32 -static-libgcc -static-libstdc++` (except you can choose a new name instead of TCPIP_SocketClient.exe) and then press enter.  Now you should have your newly compiled .exe file in the same directory.
...
### Future Development:
Currently, this code does not do anything with the responses recieved from the robot.  I may come back and improve upon that in the future.  But for now it does not add much value in terms of understanding the state of the robot which is already known in greater detail by the PLC through the EtherNet/IP communication.  But it could be of use to confirm if it is necessary to re-send the command.
